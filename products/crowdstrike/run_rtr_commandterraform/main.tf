...
resource "tines_agent" "containment_successful_0" {
    name = "Containment Successful"
    agent_type = "Agents::TriggerAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.post_message_to_a_slack_channel_3.id]
    agent_options = jsonencode({"rules": [{"path": "{{.contain_device.status}}", "type": "field==value", "value": "202"}]})
}

resource "tines_agent" "containment_unsuccessful_1" {
    name = "Containment Unsuccessful"
    agent_type = "Agents::TriggerAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.post_message_to_a_slack_channel_2.id]
    agent_options = jsonencode({"rules": [{"path": "{{.contain_device.status}}", "type": "field!=value", "value": "202"}]})
}

resource "tines_agent" "post_message_to_a_slack_channel_2" {
...
resource "tines_agent" "post_message_to_a_slack_channel_3" {
    name = "Post Message to a Slack Channel"
    agent_type = "Agents::HTTPRequestAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.prompt_response_9.id]
...
resource "tines_agent" "remove_containment_4" {
...
resource "tines_agent" "contain_device_5" {
    name = "Contain Device"
    agent_type = "Agents::HTTPRequestAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.build_results_7.id]
...
resource "tines_agent" "build_results_7" {
    name = "Build Results"
    agent_type = "Agents::EventTransformationAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.containment_successful_0.id, tines_agent.containment_unsuccessful_1.id]
...
resource "tines_agent" "receive_host_details_8" {
    name = "Receive Host Details"
    agent_type = "Agents::WebhookAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.contain_device_5.id]
    agent_options = jsonencode({"secret": "4eb0108d09a0df52c26b1f5a0d8dde91", "verbs": "get,post"})
}

resource "tines_agent" "prompt_response_9" {
    name = "Prompt Response"
    agent_type = "Agents::TriggerAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.remove_containment_4.id]
