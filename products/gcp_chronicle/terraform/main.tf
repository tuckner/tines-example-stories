...
resource "tines_agent" "search_for_ip_address_in_abuse_ipdb_0" {
...
resource "tines_agent" "webhook_agent_1" {
    name = "Webhook Agent"
    agent_type = "Agents::WebhookAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.lookup_ip_in_greynoise_4.id]
...
resource "tines_agent" "search_for_ip_address_in_virustotal_3" {
    name = "Search for IP Address in VirusTotal"
    agent_type = "Agents::HTTPRequestAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.check_ip_reputation_using_apivoid_5.id]
...
resource "tines_agent" "lookup_ip_in_greynoise_4" {
    name = "Lookup IP in GreyNoise"
    agent_type = "Agents::HTTPRequestAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.get_tor_nodes_7.id]
...
resource "tines_agent" "check_ip_reputation_using_apivoid_5" {
    name = "Check IP Reputation using APIVoid"
    agent_type = "Agents::HTTPRequestAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.search_for_ip_address_in_abuse_ipdb_0.id]
...
resource "tines_agent" "trigger_if_tor_node_6" {
    name = "Trigger if Tor Node"
    agent_type = "Agents::TriggerAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.get_ip_address_reputation_details_in_talos_intelligence_8.id]
    agent_options = jsonencode({"emit_no_match": "true", "rules": [{"path": "", "type": "regex", "value": ""}]})
}

resource "tines_agent" "get_tor_nodes_7" {
    name = "Get Tor Nodes"
    agent_type = "Agents::HTTPRequestAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.trigger_if_tor_node_6.id]
...
resource "tines_agent" "get_ip_address_reputation_details_in_talos_intelligence_8" {
    name = "Get IP Address Reputation Details in Talos Intelligence"
    agent_type = "Agents::HTTPRequestAgent"
    story_id = var.story_id
    keep_events_for = 0
    source_ids = []
    receiver_ids = [tines_agent.search_for_ip_address_in_virustotal_3.id]
